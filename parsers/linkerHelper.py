...
def generate_var(croot, filename):
    ...
    with open(outfile, "w") as f:
        ...
        for var in croot.findall('STATICROOT/TRANSLATION_UNIT/VAR_DECL[@address]'):
        ...
        # now broaden the search
        # for var in croot.findall('file/STATICROOT/TRANSLATION_UNIT/VAR_DECL[@is_definition="False"]'):
        #     if "mangled_name" in var.attrib:
        #         var_name = var.attrib['mangled_name']
        #     else:
        #         var_name = var.attrib['spelling']
        ...
        for func in croot.findall('.//FUNCTION_DECL'):
            ...
            s = ""
            for var in func.findall('.//VAR_DECL[@address]'):
                s += var.attrib['address'] + '\t'
        ...
        for c in croot.findall('.//CLASS_DECL'):
            ...
            s = ""
            for var in c.findall('./VAR_DECL[@address]'):
                s += var.attrib['address'] + '\t'
                s += helper(var, "CLASSSTATIC", class_name)
            if(len(s) > 0):
                f.write(s)

        # Get all static variables from structures
